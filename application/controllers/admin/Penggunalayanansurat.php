...
class Penggunalayanansurat extends CI_Controller
{
    ...
    function changepassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user = $this->M_userpenduduk->findByNoKTP($this->input->post('no_ktp'));

            if (!$user) {
                $this->session->set_flashdata('error', 'No KTP tidak cocok');

                redirect('admin/penggunalayanansurat');
            }

            $newPw = password_hash($this->input->post('new_password'), PASSWORD_DEFAULT);
            $this->M_userpenduduk->updatePassword($newPw, $this->input->post('no_ktp'));

            $this->session->set_flashdata('success', 'Kata sandi dari No KTP ' . $this->input->post('no_ktp') . ' telah diubah');
