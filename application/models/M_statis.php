...
class M_statis extends CI_Model
{
    ...
    {
        return $hsl = $this->db->get_where('tb_statis', array('id' => '8'));
    }
    function get_all_bpd()
    {
        return $hsl = $this->db->get_where('tb_statis', array('id' => '8'));
    }
    function get_all_statistikpenduduk()
    {
        return $hsl = $this->db->get_where('tb_statis', array('id' => '9'));
    }
    ...
    function update_apbdes($data, $kode)
    {
        $this->db->where('id', $kode);
        return $this->db->update('tb_statis', $data);
    }
    function update_bpd($data, $kode)
    {
        $this->db->where('id', $kode);
        return $this->db->update('tb_statis', $data);
    }

    function update_statistikpenduduk($data, $kode)
    {
        $this->db->where('id', $kode);
        return $this->db->update('tb_statis', $data);
    }
